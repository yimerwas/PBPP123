...
results_sigma[, c("bias", "rmse", "coverage") := .(median - sigma, (median- sigma)^2, as.numeric(sigma > q5 & sigma < q95))]
results_sigma[, c("bias", "rmse", "coverage") := .(mean(bias), sqrt(mean(rmse)), mean(coverage)), by = c("name", "scenario", "prior_a0")]
...
# Coverage plot
plot_sigma_coverage <- ggplot(plotdata1, aes(x = prior_a0, y = coverage * 100, group = name)) +
  geom_line(aes(color = name), linetype = "solid", size = 1) +
  geom_point(aes(color = name)) + 
...
# RMSE plot 
plot_sigma_rmse <- ggplot(plotdata1, aes(x = prior_a0, y = rmse, group = name)) +
  geom_line(aes(color = name), linetype = "solid", size = 1) +
  geom_point(aes(color = name)) + 
  labs(x = "Fixed a0", y = "RMSE", color = "", title = "PBPP1") +  
  scale_color_brewer(palette = "Set1") +
  theme_pub(axis.text.x = element_text(angle=45, vjust=1, hjust=1)) +
  theme(legend.position = "none") 
...
combined_plot <- plot_sigma_bias + plot_sigma_coverage + plot_sigma_rmse + legend_plot + plot_layout(ncol = 2)  
...
# Coverage plot
plot_sigma_coverage <- ggplot(plotdata23, aes(x = prior_a0, y = coverage * 100, group = scenario)) +
  facet_wrap(facets = c("name"), nrow = 1, scales = "free_y") +
  geom_line(aes(color = scenario), linetype = "solid", size = 1) +
  geom_point(aes(color = scenario)) + 
...
# RMSE plot
plot_sigma_rmse <- ggplot(plotdata23, aes(x = prior_a0, y = rmse, group = scenario)) +
  facet_wrap(facets = c("name"), nrow = 1, scales = "free_y") +
  geom_line(aes(color = scenario), linetype = "solid", size = 1) +
  geom_point(aes(color = scenario)) + 
  labs(x = "Prior a0", y = "RMSE", color = "", title = "") +
  scale_color_brewer(palette = "Set1") +
  theme_pub(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  theme(legend.position = "none", plot.margin = margin(4, 4, 4, 4))  

# Combine all plots into one
combined_plot_23 <- plot_sigma_bias + plot_sigma_coverage + plot_sigma_rmse + 
  plot_layout(ncol = 1)  
